...
:-op(500,xfx,->).    %  conditional effect world(C)->H
                     ...
                     %  diff() is defined as built-in
                     %  (no need to define it in your program)

                     %  This planner supports forbidden states
                     %
                     %  example:
                     %  forbidden_state([on(b,a),on(b,table)]).
                     %  forbidden_state([on(b,table),on(c,table)]).
                     %
                     %  The argument is a list of properties. Here "," means AND
                     %
                     %  if there are no forbidden states, write in your program
                     %      no_forbidden_state.
